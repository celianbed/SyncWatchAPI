...
@pytest.fixture()
def diffusion_a_venir(client, jeton, db, aujourdhui):
    ...
    episode.date_diffusion = aujourdhui + timedelta(days=3)
    db.commit()
    return episode


def test_calendrier_diffusion_a_venir(client, jeton, diffusion_a_venir, aujourdhui):
    ...
    assert entrees[0]["episode"]["date_diffusion"] == str(aujourdhui + timedelta(days=3))
...
def test_calendrier_serie_non_suivie(client, jeton, db, aujourdhui):
